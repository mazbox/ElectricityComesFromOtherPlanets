...
vec4 Blur(sampler2DRect tex, vec2 uv) {
	return 
			(texture2DRect(tex, uv+vec2(-1.0,-1.0)) + 
			texture2DRect(tex, uv+vec2(0.0,-1.0)) +
			texture2DRect(tex, uv+vec2(1.0,-1.0)) +
			
			texture2DRect(tex, uv+vec2(-1.0,0.0)) + 
			texture2DRect(tex, uv+vec2(0.0,0.0)) +
			texture2DRect(tex, uv+vec2(1.0,0.0)) +
			
			texture2DRect(tex, uv+vec2(-1.0,1.0)) + 
			texture2DRect(tex, uv+vec2(0.0,1.0)) +
			texture2DRect(tex, uv+vec2(1.0,1.0)))/9.0
...
void main()
{
    ...
    vec4 a = PostFX(bgl_RenderedTexture, uv);
   vec4 b = Blur(bgl_RenderedTexture, uv);
   gl_FragColor = (a+b)/2.0;
